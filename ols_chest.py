...
class ols_models():
        ...
        for region in regions:
            ...
            for cycle in (1, 7, 28): 
                if cycle == 28: ## ugh.. don't make me read this
                    error_list, mean_error_list, x_1, x_2, y_predict_list, y_actual = self.ols_stuffs(region=region, cycle=cycle, X=X)
                    error_lists.append(error_list)
                    error_lists.append(mean_error_list)
                else:
                    error_list = self.ols_stuffs(region=region, cycle=cycle, X=X)
                    error_lists.append(error_list)
            ...
            #    ols_stuffs(region=region, day=day, cycle=28, X=X, X_day=X_day)
                self.stratified_model(region=region, day=day, X=X, X_day=X_day)
            for days_of_data in (210, 420):
                error_list = self.ols_stuffs(region=region, cycle=28, X=X, days_of_data=days_of_data)
                error_lists.append(error_list)
            self.write_errors_to_csv(region, error_lists)
    ...
    def write_errors_to_csv(self,region, error_lists):
    ...
    def stratified_model(self, region, day, X, X_day):
        regr = linear_model.LinearRegression()
        
        cycle=28 
        output_file = open("ols_results/strat_output_ols_%sday_%s_%s" %(cycle, region, day,), 'w')
        ...
        while ( ((month + 2 ) * cycle) <= len(X) ):
            ...
            if len(y_predict_list_past) >0: ## this mean is used to calculate confidence intervals, mean of predictions
                mean_ = 1.0* sum(y_predict_list_past)/len(y_predict_list_past)
            else:
                mean_ = 0
            
            #mean_train = [1.0*sum(i)/cycle for i in X_train_list] # this mean is used for mean estimator, mean of training
            mean_train = (1.0 * sum(X[0:len(X_train)])) / len(X_train)
            mean_train = [mean_train] * len(Y_test)

            #import pdb; pdb.set_trace() 
            if len(y_predict_list_past) == 0: ## lame sauce
                MSE = 0
            elif day is None:
                MSE = self.calc_mse(total_error, len(y_predict_list_past)) # changed from n for monday predictor
            else:
                MSE = self.calc_mse(total_error, len(y_predict_list_past)) # changed from n for monday predictor
            ...
            output_file.write("Mean of training data: %s\n" %(mean_training[0],))
            ...
            x_1, x_2 = self.calc_confidence_intervals(mean_, sigma_, cycle, day)
            ...
            working_error = 1.0 * self.calc_relative_error(Y_test, y_predict_current_month) / 4
            mean_estimator_error = 1.0 * self.calc_relative_error(Y_test, mean_train) / 4

            total_error += self.calc_squared_error(Y_test, y_predict_current_month) 
    ...
    def ols_stuffs(self,region, day=None, cycle=28, X=None, X_day=None, days_of_data=None):
        regr = linear_model.LinearRegression()
        ...
        while ( ((month + 2 ) * cycle) <= len(X) ):
            ...
            if day is None:  ## total lameness
                working_error = 1.0 * self.calc_relative_error(Y_test, y_predict_current_month) / cycle
                mean_estimator_error = 1.0 * self.calc_relative_error(Y_test, mean_train) / cycle
            else:
                working_error = 1.0 * self.calc_relative_error(Y_test, y_predict_current_month) / 4
                mean_estimator_error = 1.0 * self.calc_relative_error(Y_test, mean_train) / 4
...
# mean_ +- (1.96 * sigma) where sigma is std of estimator and mean is mean of estimator
    ...
    def calc_squared_error(self,Y_test, y_predict_current_month):
        error = 0
        for i in range(len(Y_test)):
            error += (math.pow(1.0*Y_test[i] - y_predict_current_month[i], 2))
        return error

    def calc_relative_error(self,Y_test, y_predict_current_month):
        error = 0
        for i in range(len(Y_test)):
            try:
